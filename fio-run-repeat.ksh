...
for count in  1 ; do 
    ...
    for iosize in 64; do
	   ...
	   #while [[ $i -gt $min ]]; do
	       ...
	       #Run Stuff Here
               echo fio --output-format=normal --output=../output/output-${file} --group_reporting --section=$rw config-${file} > ${config_dir}/command-${file}
	       cat config-$count > ${config_dir}/config-${file}
               fio --output-format=normal --output=${output_dir}/output-$file  --group_reporting --section=$rw config-$count
           done 
        done
    done
